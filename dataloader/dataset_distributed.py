...
class SemKITTI_pt(data.Dataset):
    def __init__(self, data_path, cfgs, split='train', return_ref=False):
        ...
        # gh
        self.root_sam = '/home/poscoict/Desktop/SemKITTI_processed/dataset/sequences'
        ...
        # gh
        self.sam_segfea_files = []
        self.sam_imgfea_files = []
        ...
        for seq in self.seqs:
            for pcd_name in sorted(os.listdir(os.path.join(self.root, seq, 'velodyne'))):
                ...
                # gh
                self.sam_imgfea_files.append(os.path.join(self.root_sam, seq, 'img_fea', str(pcd_name[:-4]) + '.pt'))
                self.sam_segfea_files.append(os.path.join(self.root_sam, seq, 'seg_fea', str(pcd_name[:-4]) + '.pt'))
    ...
    # gh
    def _load_sam(self, index):
        imgfea_path = self.sam_imgfea_files[index]
        segfea_path = self.sam_segfea_files[index]
        imgfea = torch.load(imgfea_path, map_location='cpu')
        segfea_ = torch.load(segfea_path, map_location='cpu')
        segfea, segscore, anchorpos = segfea_[:,:,:256], segfea_[:,:,256], segfea_[:,:,257]

        return imgfea, segfea, segscore, anchorpos
    ...
    def __getitem__(self, index):
        ...
        # gh divide by max length for grid-sampling
        # normalized pixel coordinates
        max_len = max(img1.size)
        pixel_coordinates1 = pixel_coordinates1 / (max_len - 1) * 2 - 1.0
        pixel_coordinates2 = pixel_coordinates2 / (max_len - 1) * 2 - 1.0

        # gh

        # breakpoint()
        imgfea, segfea, segscore, anchorpos = self._load_sam(index)
        # get mask feature based on distance
        pts_t = torch.Tensor(pts[:, :3]) #.cuda()
        dist = torch.linalg.norm(pts_t[:, None, :] - anchorpos[None, ...], dim=2)
        dist_min, cluster_ids = dist.min(1)
        dist_mask = dist_min > 1.
        xyz_segfea = segfea[cluster_ids]
        xyz_segscore = segscore[cluster_ids]

        # cluster_ids = cluster_ids + 1
        # cluster_ids[dist_mask] = 0
        # save_obj_color_coding('tmp.obj', pts_t.cpu().numpy(), cluster_ids.cpu().numpy() % 28)

        # gh
        # assign zero values for far points from the anchor
        xyz_segfea[dist_mask] = 0
        xyz_segscore[dist_mask] = 0
        # get pixel feature
        grid_tmp = torch.Tensor(pixel_coordinates1[None, :,None, :]) #.cuda() # (1, n_pts, 1, 2)
        xyz_pixfea = torch.nn.functional.grid_sample(imgfea[None, ...], grid_tmp)[0, ..., 0].transpose(1,0)
        ...
        # img1 = self.resize(img1)  # input of image backbone?
        ...
        # valid_mask = np.array([-1] * pts.shape[0])  # todo 필요?
        ...
        # gh
        fusion_tuple = (xyz_segfea, xyz_pixfea, xyz_segscore)  # gh todo not mono?

        # if self.pix_fusion:
        #     if monocular: #如果只是单目
        #         fusion_tuple = (xyz_segfea, xyz_pixfea, xyz_segscore)  # zero padding
        #         # fusion_tuple = (camera[0], pixel_coordinates[0], masks[0], valid_mask, ori_camera[0])
        #     else:#双目
        #         fusion_tuple = (xyz_segfea, xyz_pixfea, xyz_segscore)
        #         # fusion_tuple = (camera, pixel_coordinates, masks, valid_mask, ori_camera)
        data_tuple += (fusion_tuple,)  # (data_tuple, (fusion_tuple))
...
class spherical_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # elif len(data) == 6:  # when using image features
        ...
        # todo ghlee comment if statement
        ...
        voxel_inst = insts[mask].squeeze()  # consider only thing categories
        ...
        # if type(labels) == np.ndarray and type(insts) == np.ndarray:
        return_dict['voxel_label'] = voxel_label  # (480,360,32)
        return_dict['gt_center'] = center  # (1,480,360)
        return_dict['gt_offset'] = offset  # (2,480,360)
        return_dict['inst_map_sparse'] = processed_inst != 0
        return_dict['bev_mask'] = bev_mask  # (1,480,360)
        return_dict['pt_sem_label'] = labels
        return_dict['pt_ins_label'] = insts

        # if len(data) == 6:  # fusion mode
        # todo; tmp; single level feature w/ highest score
        # breakpoint()
        row_indices = torch.arange(fusion_tuple[0].size()[0])
        max_indices = torch.argmax(fusion_tuple[2], dim=1)
        return_dict['segfea'] = fusion_tuple[0][row_indices, max_indices, :]

        # return_dict['segfea'] = fusion_tuple[0]
        return_dict['pixfea'] = fusion_tuple[1]
        return_dict['segscore'] = fusion_tuple[2]
